...
function initDatastoreConnection(config, context) {
	return function(callback) {
		...
		if (config.connectionString.substr(-1) !== '/')
	...
	if (context.s3enabled) {
		...
		*/
		r.unloadData = function(name, query, bucketName) {
			if (!name) {		
				logger.info('usage: r.unloadData(name, query /*optional*/, bucketName /*optional*/ )');
				return;			
			}
		...
		*/
		r.loadData = function(table, path, bucket, callback) {
			if (path === undefined || table === undefined) {		
				logger.info('usage: r.loadData(table, path, bucket /*optional*/, callback /*optional*/ )');
				return;			
			}
