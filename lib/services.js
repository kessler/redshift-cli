...
function initDatastoreFunctions(datastore, config, context, callback) {
	...
	if (context.s3enabled) {
		...
		*/
		r.unloadData = function(name, gzip, delimiter, query, bucketName) {
			if (!name) {		
				logger.info('usage: r.unloadData(name, gzip /*optional*/, delimiter /*optional*/, query /*optional*/, bucketName /*optional*/)');
				return;			
			}
			...
			if (typeof (config.overrideS3SecurityCredentials) === 'object') {
				...
				unloadData(datastore, config.overrideS3SecurityCredentials, config.database, name, gzip, delimiter, query, bucketName);
				...
				awsUtil.createTemporarySecurityCredentials(context.sts, function(err, tempCredentials) {
					unloadData(datastore, tempCredentials, config.database, name, gzip, delimiter, query, bucketName);	
				});
