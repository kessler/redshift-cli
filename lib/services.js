...
function showCreate(datastore, table, callback) {
	...
	var getColumnsQuery = 'select p."column" + \' \' + p."type" + CASE when p.notnull = false then \' NULL\' else \'\' end + \' ENCODE \' + DECODE(encoding,\'none\',\'raw\',encoding) as col from pg_table_def p where tablename = \'' + tablename + '\' and schemaname = \'' + schemaname + '\'';
